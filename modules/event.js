...
module.exports.load = async function(app, db) {
    ...
    app.get("/api/halloween/status", async (req, res) => {
    ...
    app.post("/api/halloween/trick-or-treat", async (req, res) => {
        if (!req.session.pterodactyl) return res.status(403).json({ error: "Unauthorized" });
    ...
    app.post("/api/halloween/exchange-candy", async (req, res) => {
        if (!req.session.pterodactyl) return res.status(403).json({ error: "Unauthorized" });
    ...
    app.post("/api/halloween/interact", async (req, res) => {
        if (!req.session.pterodactyl) return res.status(403).json({ error: "Unauthorized" });
    ...
    // Claim rewards
    app.post("/api/halloween/claim-rewards", async (req, res) => {
        if (!req.session.pterodactyl) return res.status(403).json({ error: "Unauthorized" });
